...
modelDir = "output"
predDir = "predictions"
outputDir = "indicators"
figDir = "indicators/check"
...
allModels = dir(path=modelDir, pattern="model.*\\.rds")
allModels = file.path(modelDir, grep(x=allModels, pattern=domain, value=TRUE))

nullModel = grep(x=allModels, pattern="m0.0-rep00", value=TRUE)
...
null = getPredData(nullModel, predDir)
...
for(i in seq_along(allModels)) {
  ...
  indFile = gsub(x=basename(allModels[i]), pattern="model", replacement="ind")
  indFile = gsub(x=indFile, pattern="rds$", replacement="txt")
  
  if(file.exists(file.path(outputDir, indFile))) next
  
  iMod = strsplit(basename(allModels[i]), "_")[[1]][2]
  thisModel = strsplit(iMod, "-")[[1]][1]
  thisRep   = as.numeric(gsub(strsplit(iMod, "-")[[1]][2], pattern="rep", replacement=""))

  F1File = gsub(x=basename(allModels[i]), pattern="model", replacement="check1")
  F1File = gsub(x=F1File, pattern="rds$", replacement="png")

  F2File = gsub(x=basename(allModels[i]), pattern="model", replacement="check2")
  F2File = gsub(x=F2File, pattern="rds$", replacement="png")
  
  thisPred = getPredData(allModels[i], predDir)
