...
inputDir = "output/final/indicators"
...
x[, g3] = pmax(0, as.numeric(x[, g3][[1]]))
x[, g3] = (1 - x[, g3]) # change to "unexplained deviance" 0-40%

x$ind27 = pmin(x$ind27, 10)

mx = aggregate(x, by = list(indicators$model[included]), FUN=mean)
names(mx)[1] = "model"

labs = gsub(names(mx[,-1]), pattern="ind", replacement="")

ylim = c(rep(5,3), rep(1,3), 2, 15, 1, rep(0.2, length(g3)))
ylim = rbind(0, ylim)

ylim0 = c(rep(5,3), 1, 1, 1, 2, 20, 1, rep(1, length(g3)))
ylim0 = rbind(0, ylim0)

ylim1 = c(rep(5,3), 5, 6, 5, 1, 20, 1, rep(1, length(g3)))
ylim1 = rbind(0, ylim1)
ylim0 = ylim1
...
cols = brewer.pal(7, name = "Dark2")
mfrow = if(domain=="biscay-celtic") c(2,4) else c(2,4)
