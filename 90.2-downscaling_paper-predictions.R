...
block.size = 120000 # for prediction
...
foreach(i=seq_along(allModels), .inorder = FALSE, .packages = c("scam", "mgcv", "stats", "nctools")) %dopar% {
  ...
  if(!isTRUE(testOnly)) {
    ...
    # historical data
    outputFile = gsub(x=basename(allModels[i]), pattern="model", replacement="predH")
    ...
    # future data
    outputFile = gsub(x=basename(allModels[i]), pattern="model", replacement="predF")
    outputFile = gsub(x=outputFile, pattern="rds$", replacement="nc4")
    if(!file.exists(file.path(outputDir, outputFile))) {
    ...
    # quantile data
    outputFile = gsub(x=basename(allModels[i]), pattern="model", replacement="predQ")
    if(!file.exists(file.path(outputDir, outputFile))) {
      pred = predict(mod, newdata=nenv, block.size=block.size)
      dimnames(pred) = NULL
